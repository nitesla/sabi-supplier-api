...
public class SupplierUserController {
    ...
    /** <summary>
     * Supplier User creation endpoint
     * </summary>
     * <remarks>this endpoint is responsible for creation of new Supplier user</remarks>
     */
    ...
    /** <summary>
     * Supplier user update endpoint
     * </summary>
     * <remarks>this endpoint is responsible for updating Supplier user</remarks>
     */

    @PutMapping("")
    public ResponseEntity<Response> updateSupplierUser(@Validated @RequestBody  SupplierUserDto request){
        HttpStatus httpCode ;
        Response resp = new Response();
        SupplierUserResponseDto response = service.updateSupplierUser(request);
        resp.setCode(CustomResponseCode.SUCCESS);
     ...
     * </summary>
     * <remarks>this endpoint is responsible for getting a single user</remarks>
     */
    @GetMapping("/{id}")
    public ResponseEntity<Response> getSupplierUser(@PathVariable Long id){
        HttpStatus httpCode ;
        Response resp = new Response();
        SupplierUserResponseDto response = service.findSupplierUser(id);
        resp.setCode(CustomResponseCode.SUCCESS);
     ...
     */
    @GetMapping("")
    public ResponseEntity<Response> getSupplierUser(@RequestParam(value = "userId",required = false)Long userId,
        ...
        HttpStatus httpCode ;
        Response resp = new Response();
        Page<SupplierUser> response = service.findAll(userId, wareHouseId,roleId,PageRequest.of(page, pageSize));
        resp.setCode(CustomResponseCode.SUCCESS);
        resp.setDescription("Record fetched successfully !");
        resp.setData(response);
        httpCode = HttpStatus.OK;
    ...
    public ResponseEntity<Response> getAll(@RequestParam(value = "isActive")Boolean isActive){
        HttpStatus httpCode ;
        Response resp = new Response();
        List<SupplierUser> response = service.getAll(isActive);
        resp.setCode(CustomResponseCode.SUCCESS);
        resp.setDescription("Record fetched successfully !");
        resp.setData(response);
        httpCode = HttpStatus.OK;
